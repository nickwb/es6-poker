...
class ScoreInfo {
    ...
    constructor(hand, handType) {
        let match = hand.isFullHand() && handType.doesHandMatch(hand);
        this.points = match ? handType.getScorePoints(hand) : 0;
        this.description = match ? handType.describeHand(hand) : '';
        this.name = match ? handType.name : '';
    }
	...
	truncate(handType = false) {
		return handType
			? HandType.truncateScoreToHandType(this.points)
			: HandType.truncateScoreToMultipleRanks(this.points);
	}
}
